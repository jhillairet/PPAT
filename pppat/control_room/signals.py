...
# Example of diag description file for a WEST pulse, 
# as generated by ldp command line such as :
# ldp -c55799 -D
LDP_EXAMPLE = RESOURCES_PATH / 'ldp/Choc_055799_D.ldp'

# Server to execute remote commands on
SERVER = 'talitha.intra.cea.fr'

# List of WEST signals "manually" compiled (with love). 
# These signals mix signals from ARCAD database (Tore Supra era) or IMAS
signals = {
    ## Shot date and time
...
def Prad_pradwest(pulse):
    ...
    from pppat.control_room.pradwestc import pradwest1
    try:
...
def arcad_diags(filename=LDP_EXAMPLE):
    """
    Return a dictionnary of ARCAD diagnostic names and their associated signals

    Parameters
    ----------
    filename : str, optional
        File path of a ldp file

    Returns
    -------
    diags : dict
        Dictionnary of dianostics, which contains:
        diags['DXXXX']['status'] : 'present' or 'absent'
        diags['DXXXX']['signals'] : list

    """
    diags = nested_dict()
    
    # parse the LDP file and collect the signal names 
    # for an associated diagnostics. 
    # We assume that:
    # - Diagnostic names start with "D" 
    # - signal names start by either "G" (groups) or "S" or "F"
    with open(filename) as file:
        cur_diag = None
        for line in file.readlines():
            words = line.split()
            if words:
                # diag name and status
...
def add_arcad_signals(signals_dict):
    """
    Add the ARCAD signals to the signals dictionnary
